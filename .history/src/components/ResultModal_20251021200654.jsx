...
export default function ResultModal({ ref, targetTime, remainingTime, onReset }) {
  ...
  // فقط وقتی بازنده نیست
  const score = useMemo(() => {
    if (userLost) return null;
    return Math.round((1 - (remainingTime / (targetTime * 1000))) * 100);
  }, [remainingTime, targetTime, userLost]);
  ...
  return createPortal(
    <dialog ref={dialog} className="result-modal" onClose={onReset}>
      ...
      <p>
        زمان رو وقتی <strong>{formattedRemainingTime} ثانیه مونده بود متوقف کردی.</strong>
      </p>
      <form action="dialog" onSubmit={onReset}>
        <button>بستن</button>
      </form>
